...
*/
	...
	User Function IFATR03()
		...
		DEFINE MSDIALOG oDlg TITLE "Sticker Por Pedido" From aSize[7],0 To aSize[6],aSize[5] OF oMainWnd PIXEL
	                                           
		@ 10,10   Say "Filial:"   	FONT oFont SIZE 60,10 OF oDlg PIXEL
		@ 10,140  Say "Origem:"   	FONT oFont SIZE 80,10 OF oDlg PIXEL
		@ 10,240  Say "Pedido:"   	FONT oFont SIZE 80,10 OF oDlg PIXEL
		@ 10,330  Say "Porta Impressão:"   	FONT oFont SIZE 90,10 OF oDlg PIXEL
		
	    @ 10,25  MsGet _FL 		Picture "@!" 	Size 07,10 of oDlg PIXEL FONT oFont F3 "SM0" Valid fPsqFL()
	    @ 10,65  MsGet _Filial 	Picture "@!" 	Size 70,10 of oDlg PIXEL FONT oFont When .F.
	    @ 10,165 MSCOMBOBOX oComboBo1 VAR _Orig ITEMS {"CallCenter","Faturamento"} SIZE 060, 010 OF oDlg PIXEL
		@ 10,265 MsGet _Ped 	Picture "@!" 	Size 60,10 of oDlg PIXEL FONT oFont Valid IfatR03A()
	    @ 10,385 MSCOMBOBOX oComboBo2 VAR _Porta ITEMS {"LPT1","LPT2","LPT3","LPT4","LPT5"} SIZE 060, 010 OF oDlg PIXEL
		lOk := .F.	
	...
	Static Function montaAcols()
		...
		Endif
	   	...
	   	oGetTM1 := MsNewGetDados():New(30, 0, 250, 500,GD_UPDATE, "AllwaysTrue", "AllwaysTrue", "", aEdit, , , , , , oDlg, aHeader1, aCols)
    ...
    Static Function IFATR03B()
		...
		Private CONTFL     	:= 01
		Private m_pag      	:= 01
		Private wnrel      	:= "FESTR13" // Coloque aqui o nome do arquivo usado para impressao em disco
		Private aLinhas 		:= {}
		Private cString 		:= "SB1"
		...
		EndIf   */
 		...
 		//Apaga registros da tabela auxiliar PA1 e acordo com o codigo do usuario
		_cQuery := " DELETE " + RetSqlName("PA1")
		_cQuery += " WHERE PA1_FILIAL = '" + xFilial("SB1") + "' "
		_cQuery += " AND PA1_USER = '" + __cUserID + "' "
		TCSqlExec(_cQuery)
			...
			For nX := 1 To Len(oGetTM1:aCols)
				If oGetTM1:aCols[nX][_nQuant] > 0
					For nY := 1 To oGetTM1:aCols[nX][_nQuant]
                		DbSelectArea("SB1")
                		DbSetOrder(1)
                		DbSeek(xFilial("SB1") + oGetTM1:aCols[nX][_nProd])
				
						cCodP   := AllTrim(SB1->B1_COD) 
						cUnid  	:= cCodP + " Uni:" + Alltrim(Str(SB1->B1_QE)) + " " + SB1->B1_UM 
						cDescr 	:= AllTrim(SB1->B1_DESC) + IIF(SB1->B1__SEGISP=="1","-Para Bicicleta /","" )
						cComp	:= IIF(!Empty(SB1->B1__DESCET),"/ Comp: " + AllTrim(SB1->B1__DESCET),"")
						cOrig	:= IIF(SB1->B1_ORIGEM <> "0","IMPORT.: ","FORNEC.: ") + AllTrim(cEmp)
						If SB1->B1_TIPO $ "CA/PN"
							cValid 	:= "Validade 5 anos          Garantia: 90 dias"
						Else
							cValid 	:= "Validade Indeterminada   Garantia: 90 dias"
						EndIf

						RecLock("PA1",.T.)						
							PA1->PA1_USER	:= __cUserID
							PA1->PA1_COD	:= cCodP
							PA1->PA1_DESC	:= Alltrim("*" + cCodP + "*")
							PA1->PA1_DESCI	:= cComp
							PA1->PA1_DESC1	:= cUnid
							PA1->PA1_DESC2 	:= cDescr
							PA1->PA1_DESC3	:= cOrig
							PA1->PA1_DESC4	:= cCNPJ
							PA1->PA1_DESC6	:= cValid
						PA1->(MsUnlock())
...
/*    	If _Orig == "CallCenter"
	        ...
	        mSQL :="SELECT UB_PRODUTO,B1_DESC,UB_QUANT,B1_QE,B1_UM,B1__DESCET,B1_TIPO,B1_ORIGEM,B1__SEGISP,B1__DESCET,UB_FILIAL "
			mSQL +=" FROM "+RetSqlName("SUB")+" SUB,"+RetSqlName("SB1")+" SB1 "
			mSQL +=" WHERE UB_NUM ='"+_Ped+"'  AND UB_FILIAL='"+AllTrim(_FL)+"' AND SUB.D_E_L_E_T_ <>'*' "
        	mSQL +=" AND B1_COD=UB_PRODUTO and B1_FILIAL='"+xFilial("SB1")+"' AND SB1.D_E_L_E_T_ <>'*' "
	   		...
	   		If TMP->(!Eof())
				...
				While TMP->(!EOF()) 
					...
					For H := 1 TO TMP->UB_QUANT
						...
						mSQL := "SELECT ZE_FILIAL,ZE_NOMECOM,ZE_CGC FROM "+RetSqlName("SZE")+" WHERE  D_E_L_E_T_ <>'*' "
						mSQL += "AND ZE_CODIGO='"+SM0->M0_CODIGO+"' AND ZE_CODFIL='"+TMP->UB_FILIAL+"'"
						
					    DbUseArea(.T.,"TOPCONN",TcGenQry(,,mSQL),"TSZE",.F.,.T.)
						TSZE->( DbGoTop() )
						If TSZE->(!Eof())
						    cEmp    := TSZE->ZE_NOMECOM
						    cCNPJ	:= TSZE->ZE_CGC
					    Endif
						TSZE->(dbCloseArea())	
			                  
                      
						cCodP   := AllTrim(TMP->UB_PRODUTO) 
						cUnid  	:= cCodP+" Uni:"+Str(TMP->B1_QE)+" "+TMP->B1_UM 
						cDescr 	:= AllTrim(TMP->B1_DESC)+IIF(TMP->B1__SEGISP=="1","-Para Bicicleta /","" )+IIF(!Empty(TMP->B1__DESCET),"/ Comp: "+AllTrim(TMP->B1__DESCET),"")
						cOrig	:= IIF(TMP->B1_ORIGEM <> "0","IMPORT.: ","FORNEC.: ")+AllTrim(cEmp)
	                    cCNPJ	:= "CNPJ:"+cCNPJ
						If TMP->B1_TIPO $ "CA/PN"
							cValid 	:= "Validade 5 anos          Garantia: 90 dias"
						Else
							cValid 	:= "Validade Indeterminada   Garantia: 90 dias"
						EndIf
						...
						AADD(_aPRODUTO ,{cCodP,cDescr,cUnid,cOrig,cCNPJ,cValid })
		...
		Else
	        ...
	        mSQL :="SELECT C6_PRODUTO,B1_DESC,C6_QTDVEN,C6_FILIAL,B1_QE,B1_UM,B1__DESCET,B1_TIPO,B1_ORIGEM,B1__SEGISP,B1__DESCET,C6_FILIAL "
			mSQL +=" FROM "+RetSqlName("SC6")+" SC6,"+RetSqlName("SB1")+" SB1 "
			mSQL +=" WHERE C6_NUM ='"+_Ped+"'  AND C6_FILIAL='"+AllTrim(_FL)+"' AND SC6.D_E_L_E_T_ <>'*' "
        	mSQL +=" AND B1_COD=C6_PRODUTO and B1_FILIAL='"+xFilial("SB1")+"' AND SB1.D_E_L_E_T_ <>'*' "
	   		...
	   		If TMP->(!Eof())
				...
				While TMP->(!EOF()) 
					...
					For H := 1 TO TMP->C6_QTDVEN
						...
						mSQL := "SELECT ZE_FILIAL,ZE_NOMECOM,ZE_CGC FROM "+RetSqlName("SZE")+" WHERE  D_E_L_E_T_ <>'*' "
						mSQL += "AND ZE_CODIGO='"+SM0->M0_CODIGO+"' AND ZE_CODFIL='"+TMP->C6_FILIAL+"'"
						...
						cCodP   := AllTrim(TMP->C6_PRODUTO) 
						cUnid  	:= cCodP+" Uni:"+AllTrim(Str(TMP->B1_QE))+" "+TMP->B1_UM 
						cDescr 	:= AllTrim(TMP->B1_DESC)+IIF(TMP->B1__SEGISP=="1","-Para Bicicleta /","" )+IIF(!Empty(TMP->B1__DESCET),"/ Comp: "+AllTrim(TMP->B1__DESCET),"")
						cOrig	:= IIF(TMP->B1_ORIGEM <> "0","IMPORT.: ","FORNEC.: ")+AllTrim(cEmp)
	                    cCNPJ	:= "CNPJ:"+cCNPJ
						If TMP->B1_TIPO $ "CA/PN"
	...
	CallCrys('IFATCR12',__cUserID,"1;0;1;Etiqueta")	//Por Local
	...
	Static Function fGeraEtq()
		...
		Private cEtiqTEMP	:= 'C:\TEMP\IFATR03.PRN'	 		 
		Private nHdl    

		Private _aDiretorio     := Directory( "C:\TEMP\???????.PRN" ) 
        ...
        cD2 := SUBSTR(_aProduto[1,2],46,IIF(Len(_aProduto[1,2]) > 90,45,Len(_aProduto[1,2])) )
        cD3 := SUBSTR(_aProduto[1,2],90,Len(_aProduto[1,2]))
		...
		//WaitRun("cmd /c copy C:\TEMP\IFATR03.PRN "+_Porta)
		cCMD := "cmd /c type C:\Temp\etiqlote.xx > "+_Porta
		Winexec(cCMD)   
	Return

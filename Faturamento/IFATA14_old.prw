...
If _lCredLoj
	...
	_cQuery += "                        ADE.ADE_CHAVE = SA1.A1_COD || SA1.A1_LOJA AND             " + CRLF
...
If(M->UA__DESINI > 0 .AND. M->UA__DESINI != M->UA_DESC1)
	AADD(aBloq,{M->UA__FILIAL,;
	_cNum,;
	"",;
	"",;
...
For nX := 1 To Len(aCols)
	If(!aCols[nX][Len(aHeader)+1])
		...
		EndIf
		//motivo 79 - DESCONTO DE PROMOCAO EM DESACORDO                           
		If(TRB_SUB->ACP_FAIXA > 0 .AND. (aCols[nX][_nPosQt] < TRB_SUB->ACP_FAIXA) )
			AADD(aBloq,{M->UA__FILIAL,;
			_cNum,;
			aCols[nX][_nCodItem],;
			aCols[nX][_nPosCod],;
...
/*	DbSelectArea("Z05")
	...
	If _lAuto
		Reclock("SUA",.F.)
			...
			EndIf
			SUA->UA__SITCOM := IIF(!lCom,"A","E") //IIF(Empty(SUA->UA__SITCOM),"A","E")
			SUA->UA__SITFIN := IIF(!lFin,"A","E") //IIF(Empty(SUA->UA__SITFIN),"A","E")
	...
	Else
		...
		EndIf
		M->UA__SITCOM := IIF(Empty(M->UA__SITCOM),"A","E")
		M->UA__SITFIN := IIF(Empty(M->UA__SITFIN),"A","E")

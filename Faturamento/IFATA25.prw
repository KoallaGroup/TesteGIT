...
DbSelectArea( "SL4" )
If DbSeek( SUA->UA_FILIAL + SUA->UA_NUM + "SIGATMK ")
	While !Eof() 					  		.AND. ;
		  SL4->L4_FILIAL == SUA->UA_FILIAL  .AND. ;
...
If DbSeek(SUA->UA_FILIAL + SUA->UA_NUM )
	While !Eof() .AND. SUA->UA_FILIAL == SUB->UB_FILIAL .AND. SUB->UB_NUM == SUA->UA_NUM
		Reclock("SUB" ,.F.,.T.)
		DbDelete()
		MsUnlock()
		SUB->(DbSkip())
...
DbSetorder(1)
If DbSeek(SUA->UA_FILIAL + SUA->UA_NUM )
	While !Eof() .AND. ZUB->ZUB_FILIAL == SUA->UA_FILIAL .AND. ZUB->ZUB_NUM == SUA->UA_NUM
		Reclock("ZUB" ,.F.,.T.)
		DbDelete()
		MsUnlock()
		ZUB->(DbSkip())

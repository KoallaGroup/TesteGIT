...
If /*l103Class .And.*/ SF1->F1__WMSRET == '1' .And. nConfirma == 1
	...
	if DbSeek(xFilial("SD1") + SF1->F1_DOC + SF1->F1_SERIE + SF1->F1_FORNECE + SF1->F1_LOJA)
		...
		While !Eof() .AND. SD1->D1_FILIAL == xFilial("SD1") .AND. SD1->D1_DOC == SF1->F1_DOC .AND. SD1->D1_SERIE == SF1->F1_SERIE.AND. SD1->D1_FORNECE == SF1->F1_FORNECE .AND. SD1->D1_LOJA == SF1->F1_LOJA
...
if nOpcao == 3 .and. _nCount == 0 .ANd. SF1->F1_TIPO == "D"		// inclusão
	...
	if nConfirma == 1
		...
		if SA1->A1_SIMPNAC == '1' .and. SF1->F1_FORMUL == "S"
...
if empty(_cNota) .or. empty(_cSerie) .or. empty(_dEmissao)
	...
	_nPosD1_NFORI	:= ASCAN(aHeader, { |x| AllTrim(x[2]) == "D1_NFORI" })
	_nPosD1_ITORI	:= ASCAN(aHeader, { |x| AllTrim(x[2]) == "D1_SERIORI" })
	...
	for x :=1 to len(aCols)
		...
		AADD(_aLinha,{"D1_NFORI"	   		,aCols[x][_nPosD1_NFORI]	,NIL})
		AADD(_aLinha,{"D1_SERIORI"	   		,aCols[x][_nPosD1_ITORI]	,NIL})

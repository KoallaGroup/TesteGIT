...
Private _cFornAte	:= Replicate('Z',TamSX3("A2_COD")[1])
Private _dPedDe		:= Date() - 365 //CTOD("  /  /    ")
Private _dPedAte	:= Date() + 365 //CTOD("  /  /    ")
...
Private _cNmKit		:= ""
Private _dMesDe		:= FirstDay(Date()) //CTOD("  /  /    ")
Private _dMesAte	:= LastDay(Date()) //CTOD("  /  /    ")
Private _dAnoDe		:= CTOD("01/01/" + Alltrim(Str(Year(Date())))) //CTOD("  /  /    ")
Private _dAnoAte	:= CTOD("31/12/" + Alltrim(Str(Year(Date())))) //CTOD("  /  /    ")
Private lChk1		:= .F.
Private lChk2		:= .F.
...
@ _nLinha,aStru[1][1] CheckBox oChkMar2 Var lChk2  Prompt "" Size 075,007 Of oDlgTMP PIXEL
oChkMar2:bChange := {|| If(lChk2,oKit:Enable(),oKit:Disable()),oKit:Refresh() }
@ _nLinha,(aStru[1][1]+12) Say "Kit"			SIZE 040,10 OF oDlgTMP PIXEL FONT oFont 
@ _nLinha,aStru[1][2] MsGet oKit Var _cKit	 	SIZE 020,10 of oDlgTMP PIXEL FONT oFont F3 "SUG_AC" VALID (Valkit(_cKit))
oKit:Disable()
...
Local cParams	:= ""	
Local _cCurva	:= IIF(lChk1,' CURVAABC DESC',' B1_DESC')

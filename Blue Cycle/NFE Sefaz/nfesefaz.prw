...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			If !Empty(SF2->F2_TRANSP)
				...
				If !Empty(SA4->A4_XCOD)
					cMensFis += "Redespacho: "+Posicione("SA4",1,xFilial("SA4")+SA4->A4_XCOD,"A4_NOME");
							 +" - "+SA4->A4_CGC+" - "+SA4->A4_END+" - "+SA4->A4_BAIRRO+" - "+SA4->A4_MUN+" - "+SA4->A4_EST
				EndIf                                                                           								 
			...
			If !Empty(SA1->A1_ENDENT)
				cMensFis += "Endereço Entrega: "+SA1->A1_ENDENT;
						 +" - "+SA1->A1_BAIRROE+" - "+SA1->A1_MUNE+" - "+SA1->A1_ESTE
			EndIf                                                                           								 			
			...
			If !Empty(SF2->F2_DUPL) .And. ALLTRIM(SF2->F2_CLIENTE) <= '900000'	
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN == "VD"
					   ...
					   //	nDesconto := 0  //Alterado por Valdemir do Carmo em 18/11 para atender uma solicitação da Contabilide para desconsiderar os descontos na transmissão da nota
					...
					cDescProd := ALLTRIM(IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)) + " PN:"+ALLTRIM(SB1->B1_PN)
				    ...
				    If SFT->(DbSeek(xFilial("SFT")+"S"+(cAliasSD2)->(D2_SERIE+D2_DOC+D2_CLIENTE+D2_LOJA+PadR(D2_ITEM,TamSx3("FT_ITEM")[1])+D2_COD)))
					   ...
					   If SFT->(FieldPos("FT_PRINCMG")) > 0 .And. SFT->(FieldPos("FT_VLINCMG")) > 0
						...
						EndIf
					EndIf 
					If Substr(SFT->FT_CLASFIS,2,2)  $  "00-30-40-41-50-10-60" .And. SFT->FT_DESCZFR>0 
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				cDescProd := ALLTRIM(SB1->B1_DESC )+" PN:"+ALLTRIM(SB1->B1_PN)
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					(cAliasSD1)->D1_VALFRE,;
					(cAliasSD1)->D1_SEGURO,;
					(nDescRed + nDescIcm) + (IIF((cAliasSD1)->D1_TIPO $ "D" .AND.(cAliasSD1)->D1_DESCZFR > 0,(cAliasSD1)->D1_VALDESC-(cAliasSD1)->D1_DESCZFR,(cAliasSD1)->D1_VALDESC)),;
...
Do Case
    ...
    documento fiscal referenciado conforme  NT 2013/005 v 1.20.
   	*/
   	Case (aNota[5]) $ "D" .and. Empty(aNfVinc).and. Alltrim(SFT->FT_CFOP) $ "1201-1202-1410-1411-5921-6921-1204-2204-1910-2910-1949-2949-1914-5202-2202"
   	      cTPNota:= "4" 
...
If cVeramb >= "3.10"
	...
	Else               
		...
		Else
			...
			cIndFinal:= "0"
		EndIf	
//		cString += '<indFinal>0</indFinal>'//0-Não Alterado por Valdemir em 13/04
//		cIndFinal:= "0"
	EndIf
	If Empty(cIndPres)
		If aNota[5] == "N"
...
//cString += '<vUnCom>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[16],21,8))+'</vUnCom>'  Alterado por Valdemir do Carmo em 31/03/16
...
If cVerAmb >= "3.10" .and. Len(aICMSZFM) > 0 .And. Len(aCST) > 0 .And. !Empty(aICMSZFM[1]) .And. (aCST[1] $ '30,40,41,50,00,10,60')  
	If !(lMvNFLeiZF)	
...
If Len(aDI)>0 .And. ConvType(aDI[04][1]) == "I19"
	...
	If lDInoDanfe
		...
		//CUSTOMIZADO OBSERVAÇAO DO VALOR DE PIS E COFINS   /THIAGO NASCIMENT            
			...
			IF SD1->(DBSeek(xFilial("SD1")+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA))
				...
				WHILE 	SD1->(!EOF()) .AND. xFilial("SD1")== SF1->F1_FILIAL .AND. SD1->D1_DOC == SF1->F1_DOC .AND. SF1->F1_SERIE == SD1->D1_SERIE ;
				.AND. SD1->D1_FORNECE == SF1->F1_FORNECE .AND. SD1->D1_LOJA == SF1->F1_LOJA
				
				nMajora := nMajora + SD1->D1_VALCMAJ
								
				SD1->(dBskip())		
				EndDO
		    ENDIF
        _nCofins:= 0
		_nCofins:= SF1->F1_VALIMP5-nMajora //Imprimir confins sem Majoração
		cMsgDI += " Valor Pis: R$ " + ConvType(SF1->F1_VALIMP6,15,2)                                                         
		cMsgDI += " Valor Cofins: R$ " + ConvType(_nCofins,15,2)
		cMsgDI += " Valor Majoração Cofins: R$ " + ConvType(nMajora,15,2)

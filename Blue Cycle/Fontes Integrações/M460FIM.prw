...
If !Empty(_aResult)
	If _aResult[1] == "S"
		...
		_lRet := .f. 
	Else
		//Help( Nil, Nil, "ENVNOTFIS", Nil, "Nota Fiscal enviada com sucesso. - " + _aResult[2], 1, 0 ) //fonte alterado para emissão do alerta apenas se houver algum erro na integração - 15/02/17
	EndIf
Else
...
DbSelectArea("SE2")
DbSetOrder(1)
If DbSeek(xFilial("SE2")+"ICM"+SF2->F2_DOC)          
	RecLock("SE2",.F.)
...
While !Eof() .And. SE1->E1_NUM == SF2->F2_DOC

        RecLock("SE1",.F.)
        SE1->E1_VENCPDD := SE1->E1_VENCREA
        MsUnlock("SE1")
...
/*       ALTERADO POR VALDEMIR DO CARMO EM 12/11 - ALTERAÇÃO PARA OPERAÇÃO BLUE CYCLE  
...
RestArea(_aArea)  ALTERADO POR VALDEMIR DO CARMO EM 12/11 - ALTERAÇÃO PARA OPERAÇÃO BLUE CYCLE  */
Return _lret
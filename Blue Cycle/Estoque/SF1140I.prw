		...
		For x:=1 To Len(aCols)   
			If !(aCols[x][Len(aCols[x])])     
				...
				If dbSeek (xFilial("SC7")+aCols[x][nPosPedido]+aCols[x][nPosItemPC])
					...
					if aCols[x][nPosIPI] != SB1->B1_IPI  .and. SA2->A2__IPI != '2'   // valida ipi - código 090				
...
/*	_cQuery := "SELECT SD1.D1_DOC,                                " + Chr(13)
	...
	If !(eof())
		If( !Empty( (_cTab)->ACLAS ) )
	    	Alert("Existem notas já classificadas com este documento de frete" + Chr(10) + Chr(13) + "Haverá divergencia no cálculo dessas notas")
		EndIf

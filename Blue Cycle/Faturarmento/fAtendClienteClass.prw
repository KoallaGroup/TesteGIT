...
Method CreateTopColumn() Class fAtendCliente
	...
	Self:aItemsResAte 	:= {"1-Contato","2-Reagendamento","3-Pedido","4-Orçamento","5-Sem Contato"}
   ...
   SA1->(!DbSeek(xFilial("SA1")+Padr(Self:cCodCli,6)+Self:cCliLoja))
	...
	If ZZ0->(!DbSeek(xFilial("ZZ0")+ Padr(Self:nIDAte,6))) //xFilial("ZZ0")+ Padr(Self:nIDAte,TAMSx3("ZZ0_IDATEN")[1]) 
		...
		ZZ0->ZZ0_OBSERV	:= Self:cObserv			// Observação do Atendimento
   		...
   		Else                    	
   			ZZ0->ZZ0_RESATE	:= cOp // Retorno Combo Resultado // {"1-Contato","2-Reagendamento","3-Pedido","4-Orçamento","5-Sem Contato"}  

...
Static Function ModelDef()
	...
	//Cria o objeto do Modelo de Dados
        //Irie usar uma função MVC003V que será acionada quando eu clicar no botão "Confirmar"
	oModel := MPFormModel():New('GrupoTributario', /*bPreValidacao*/, { | oModel | '<strong><em><span style="color: #ff0000;">MVC003V</span></em></strong>( oModel )' } , /*{ | oMdl | MVC003C( oMdl ) }*/ ,, /*bCancel*/ )
	...
	oModel:addFields('CamposSX5',,oStr1,{|oModel|'<strong><span style="color: #ff0000;"><em>MVC003T</em></span></strong>(oModel)'},,)
...
Static Function MVC003T( oModel )
	Local lRet      := .T.
...
Static Function MVC003V( oModel )
	...
	Begin Transaction
		...
		if nOpc == 3 .or. nOpc == 4
			...
			if Empty(cChave)
				SFCMsgErro("O campo chave é obrigatório!","MVC003")
				lRet := .F.
			Endif
			
			if Empty(cDescri)
				SFCMsgErro("O campo descrição é obrigatório!","MVC003")
				lRet := .F.
			Endif

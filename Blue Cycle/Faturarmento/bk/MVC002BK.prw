...
Static Function ModelDef()
	Local oModel
	Local oStr1:= FWFormStruct( 1, 'ZZ1', /*bAvalCampo*/,/*lViewUsado*/ ) //Local oStr1:= FWFormStruct( 1, 'SX5', /*bAvalCampo*/,/*lViewUsado*/ ) // Construção de uma estrutura de dados
	
	//Cria o objeto do Modelo de Dados
        //Irie usar uma função MVC002V que será acionada quando eu clicar no botão "Confirmar"
...
/*
	...
	Begin Transaction
		...
		if nOpc == 3 .or. nOpc == 4
			...
			if Empty(cChave)
				SFCMsgErro("O campo chave é obrigatório!","MVC002")
				lRet := .F.
			Endif
			
			if Empty(cDescri)
				SFCMsgErro("O campo descrição é obrigatório!","MVC002")
				lRet := .F.
			Endif

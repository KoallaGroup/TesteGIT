...
@ 10,25  MsGet _FL 		Picture "@!" 	Size l07,10 of oDlg PIXEL FONT oFont F3 "SM0" Valid fPsqFL()
@ 10,65  MsGet _Filial 	Picture "@!" 	Size 70,10 of oDlg PIXEL FONT oFont When .F.
@ 10,165 MSCOMBOBOX oComboBo1 VAR _Orig ITEMS {"CallCenter","Faturamento"} SIZE 060, 010 OF oDlg PIXEL FONT oFont
@ 10,265 MsGet _Ped 	Picture "@!" 	Size 60,10 of oDlg PIXEL FONT oFont Valid IfatR03A()
@ 10,385 MSCOMBOBOX oComboBo2 VAR _Porta ITEMS _aSep SIZE 115, 010 OF oDlg PIXEL FONT oFont
lOk := .F.
...
Private CONTFL     	:= 01
Private m_pag      	:= 01
Private wnrel      	:= "IFATR03" // Coloque aqui o nome do arquivo usado para impressao em disco
Private aLinhas 	:= {}
Private cString 	:= "SB1"
...
For nX := 1 To Len(oGetTM1:aCols)
	If oGetTM1:aCols[nX][_nQuant] > 0
		...
		cDescr 	:= AllTrim(SB1->B1_DESC) + IIF(SB1->B1__SEGISP=="1","-Para Bicicleta /","" )+IIF(!Empty(SB1->B1_DESC),"/ Comp: "+AllTrim(SB1->B1_DESC),"") //+AllTrim(SB1->B1__DESCET),"")
		cComp	:= IIF(!Empty(SB1->B1_DESC),"/ Comp: " + AllTrim(SB1->B1_DESC),"")
		...
		AADD(_aPRODUTO ,{cCodP,cDescr,cUnid,cOrig,cCNPJ,cValid,oGetTM1:aCols[nX][_nQuant]})

...
/*/  
User Function zChamada    
Local cquery := "SELECT B1_COD, B1_DESC FROM SB1010 WHERE D_E_L_E_T_ = ' ' "
Local cRetorno := "B1_COD"
Local cGroup := ""
Local cOrder := "B1_COD"


u_zConsSQL( cquery, cRetorno, cGroup, cOrder)
...
User Function zConsSQL(cConsSQLM, cRetorM, cAgrupM, cOrderM)
    ...
    Default cOrderM   := ""
    //Privates
    Private cConsSQL  := "SELECT B1_COD, B1_DESC, B1_PN FROM SB1010 WHERE D_E_L_E_T_ = ' ' " //cConsSQLM
    Private cCampoRet := "B1_COD" //cRetorM
    Private cAgrup    := "" //cAgrupM
    Private cOrder    := "B1_COD" //cOrderM
    Private nTamanRet := 0
    Private aStruAux    := {}
    //MsNewGetDados
    Private oMsNew

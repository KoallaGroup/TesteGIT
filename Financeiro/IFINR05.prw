...
/*
+-----------+---------+-------+-------------------------------------+------+---------------+
| Programa  | IFINR05 | Autor | Rubens Cruz	- Anadi Soluções 		| Data | Janeiro/2013  |
+-----------+---------+-------+-------------------------------------+------+---------------+
| Descricao | Impressão do boleto Safra													   |
+-----------+------------------------------------------------------------------------------+
| Uso       | ISAPA													 					   |
+-----------+------------------------------------------------------------------------------+
*/

User Function IFINR05(_cPref,_cParce,_cNum,_cCli,_cLoja,cPasta,_cAgen,_cConta)

LOCAL	aPergs	:= {} 
PRIVATE lExec	:= .F., cIndexName := '', cIndexKey  := '', cFilter := '', cMarca := GetMark(), lInverte := .f.
Private titulo	:= "Impressao de Boleto com Codigo de Barras", cPerg := PADR("IFINR05",Len(SX1->X1_GRUPO))
PRIVATE cTitAux	:= ""
Private lArq := .F.
default	_cPref	:= ""
default _cParce := ""
...
If Empty(_cPref)  //Geração manual do boleto
	...
	AjustaSx1(PADR("IFINR05",Len(SX1->X1_GRUPO)),aPergs)
...
Else //Geração automatica do boleto
	...
	If !(DbSeek(xFilial("SA6") + '422' + _cAgen + _cConta))
...
If Empty(_cPref)  //Geração manual do boleto
	...
	DEFINE SBUTTON OBJECT oBtn0 FROM 157,220 TYPE 1  ENABLE OF oDlg ACTION (IFINR05I("T")) 
	DEFINE SBUTTON OBJECT oBtn1 FROM 157,260 TYPE 1  ENABLE OF oDlg ACTION (IFINR05I("I"))
	DEFINE SBUTTON OBJECT oBtn2 FROM 157,300 TYPE 1  ENABLE OF oDlg ACTION (IFINR05M())
...
/*
+-----------+----------+-------+--------------------------------------+------+---------------+
| Programa  | IFINR05I | Autor | Rubens Cruz	- Anadi Soluções 	  | Data | Dezembro/2013 |
+-----------+----------+-------+--------------------------------------+------+---------------+
...
Static Function IFINR05I(_cBtn)
...
/*
+-----------+----------+-------+--------------------------------------+------+---------------+
| Programa  | IFINR05M | Autor | Rubens Cruz	- Anadi Soluções 	  | Data | Dezembro/2013 |
+-----------+----------+-------+--------------------------------------+------+---------------+
...
Static Function IFINR05M(_cPref,cPasta)
LOCAL oPrint
...
LOCAL aBolText     := {	"NÃO RECEBER APÓS O VENCIMENTO",;
						"COBRAR JUROS DE MORA DE R$ ",;
						"AO DIA APÓS O VENCIMENTO"}
...
Do While !EOF()
	...
	IF EMPTY(SE1->E1_NUMBCO)
		_cSeqNum := Posicione("SX5", 1, xFilial("SX5")+"_2"+"SAFRA", "X5_DESCRI")
	...
	_cCodBarra    	:= Ret_cBarra(Subs(aDadosBanco[1],1,3)+"97",aDadosBanco[3],aDadosBanco[4],aDadosBanco[5],AllTrim(_cSeqNum),(E1_VALOR-nVlrAbat),E1_VENCREA)//Datavalida(E1_VENCREA,.T.))
...
//		_cBolNome := "bol_" + SE1->E1_PREFIXO + "_" + SE1->E1_NUM + "_" + SE1->E1_PARCELA + "_" + SE1->E1_TIPO           
		...
		IFINR05P(oPrint,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,_cCodBar,_cLinhaDig) //aCB_RN_NN)
		nX := nX + 1 
...
/*
+-----------+----------+-------+-------------------------------------+------+---------------+
| Programa  | IFINR05P | Autor | Rubens Cruz	- Anadi Soluções 	 | Data | Dezembro/2013 |
+-----------+----------+-------+-------------------------------------+------+---------------+
| Descricao | Rotina de Impressão do boleto SAFRA 										   |
+-----------+------------------------------------------------------------------------------+
| Uso       | ISAPA																		   |
+-----------+------------------------------------------------------------------------------+
*/
Static Function IFINR05P(oPrint,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,_cCodBar,_cLinhaDig)
LOCAL oFont8
LOCAL oFont11c
LOCAL oFont10
LOCAL oFont14
...
oPrint:Say  (nRow2 + 0080,0215,"Banco Safra",oFont14)
oPrint:Say  (nRow2 + 0080,0506,"422-7",oFont24)
...
oPrint:Line (nRow2+0100,0100,nRow2+1100,0100) //Vertical
oPrint:Line (nRow2+0100,2150,nRow2+1100,2150) //Vertical
oPrint:Line (nRow2+0100,1680,nRow2+0260,1680) //Vertical
oPrint:Line (nRow2+1100,0100,nRow2+1100,2150)                                              

oPrint:Line (nRow2+0180,470 ,nRow2+0260,0470)
oPrint:Line (nRow2+0180,790 ,nRow2+0260,0790)
oPrint:Line (nRow2+0100,940 ,nRow2+0260,0940)

oPrint:Say  (nRow2+0125,105 ,"Cedente",oFont11)
oPrint:Say  (nRow2+0170,105 ,Upper(aDadosEmp[1]),oFont13n)				//Nome + CNPJ
                                
oPrint:Say  (nRow2+0125,0945,"Nosso Número"                                   ,oFont11)
cString  := Alltrim(aDadosTit[6])
nCol 	 := 0945+(374-(len(cString)*22))
oPrint:Say  (nRow2+0170,nCol,cString,oFont13n) 
...
oPrint:Say  (nRow2+0205,105 ,"Data do Documento",oFont11)
oPrint:Say  (nRow2+0250,115, StrZero(Day(aDadosTit[2]),2) +"/"+ StrZero(Month(aDadosTit[2]),2) +"/"+ Right(Str(Year(aDadosTit[2])),4), oFont13n)

oPrint:Say  (nRow2+0205,480 ,"Número do Documento",oFont11)
oPrint:Say  (nRow2+0250,500 ,aDadosTit[7]+aDadosTit[1],oFont13n) //Prefixo +Numero+Parcela

oPrint:Say  (nRow2+0205,0795,"Carteira"                                       ,oFont11)
oPrint:Say  (nRow2+0250,0795,aDadosBanco[6]                                  	,oFont13n)


oPrint:Say  (nRow2+0205,0945,"Agência/Código Cedente",oFont11)
cString := Alltrim(aDadosBanco[3])+Alltrim(aDadosBanco[7])+"/"+aDadosBanco[4]+IIF(Empty(aDadosBanco[5]),"","-"+aDadosBanco[5])
nCol 	:= 0945+(230-(len(cString)*11))
oPrint:Say  (nRow2+0250,nCol,cString,oFont13n)

oPrint:Say  (nRow2+0205,1690,"Valor"                          	,oFont11)
cString  := Transform(aDadosTit[5],"@E 99,999,999.99")
nCol 	 := 1690+(374-(len(cString)*22))
oPrint:Say  (nRow2+0250,nCol,cString,oFont13n)

oPrint:Say  (nRow2+0285,0105,"Sacado"                                         ,oFont11)
oPrint:Say  (nRow2+0330,0105,Upper(Alltrim(aDatSacado[1]))                    ,oFont13n)

oPrint:Say  (nRow2 + 0365,0105 ,"Fornecedor: " + Alltrim(aDadosEmp[1]),oFont11)
oPrint:Say  (nRow2 + 0365,1430 ,aDadosEmp[6],oFont13n)

oPrint:Say  (nRow2 + 0445,0105,"ESTE BOLETO REPRESENTA DUPLICATA CEDIDA FIDUCIARIAMENTE AO BANCO SAFRA S/A, FICANDO VEDADO O PAGAMENTO",oFont11)
oPrint:Say  (nRow2 + 0485,0105,"DE QUALQUER OUTRA FORMA QUE NÃO ATRAVÉS DO PRESENTE BOLETO",oFont11)
...
oPrint:Say  (nRow2+1120,1800,"Autenticação Mecânica",oFont10)
...
oPrint:Say  (nRow3+1685,215,"Banco Safra",oFont14 )	// 	[1]Numero do Banco
oPrint:Say  (nRow3+1685,506,"422-7",oFont24 )	// 	[1]Numero do Banco
...
oPrint:Say  (nRow3+1860,105 ,Upper(aDadosEmp[1])	,oFont13n) //Nome + CNPJ
...
oPrint:Say  (nRow3+1935,1410, DTOC(Date()), oFont13n)
...
oPrint:Say  (nRow3+1965,105 ,"Data de Operação"                                       ,oFont11)
oPrint:Say  (nRow3+1995,120, DTOC(Date()), oFont13n)
...
oPrint:Say  (nRow3+2080,105 ,aBolText[1],oFont13n)
oPrint:Say  (nRow3+2120,105 ,aBolText[2] + AllTrim(Transform( aDadosTit[5] * (aDadosBanco[8]/100),"@E 99,999,999.99")) + " " + aBolText[3],oFont13n)
...
oPrint:Say  (nRow3+2330,0215,Upper(Alltrim(aDatSacado[1]))                    ,oFont13n)
...
oPrint:Say  (nRow3 + 2375,105 ,"Endereço",oFont11)
oPrint:Say  (nRow3 + 2375,0240,aDatSacado[3] + " - " + aDatSacado[4] + " - " + aDatSacado[5],oFont13n)
oPrint:Say  (nRow3 + 2415,0240,aDatSacado[6],oFont13n)

oPrint:Say  (nRow3 + 2480,105 ,"Sacador/Avalista PROTESTAR APÓS 5 DIAS ÚTEIS",oFont11)
oPrint:Say  (nRow3 + 2580,1600,"Autenticação mecânica",oFont11)
oPrint:Say  (nRow3 + 2680,1800,"Ficha de Compensação",oFont11)
...
oPrint:FwMsBar("INT25",56.5,4,_cCodBar,oPrint,.F.,,.T.,0.017,1.01,NIL,NIL,NIL,.F.) 
//oPrint:Code128C(nRow3+2710,0150,_cCodBar, 48)

IF EMPTY(SE1->E1_NUMBCO) .AND. alltrim(aDadosBanco[1])=='422'
	DbSelectArea("SX5")
	dbSetOrder(1)
	dbSeek(xFilial("SX5")+"_2"+"SAFRA")
	While !RecLock("SX5",.F.)
	Enddo
...
s    :=  bldocnufinal//cAgencia + cConta + _cCart + bldocnufinal
dvnn := modulo10(s) // digito verifacador Agencia + Conta + Carteira + Nosso Num
NN   := bldocnufinal //+ AllTrim(Str(dvnn))
//
//	-------- Definicao do CODIGO DE BARRAS
s    := Substr(cBanco,1,4) + _cfator + blvalorfinal + Substr(cBanco,5,1) + Substr(cAgencia,1,4) + '000' + cConta + cNroDoc //Substr(cBanco,1,4) + _cfator + blvalorfinal + _cCart + bldocnufinal + AllTrim(Str(dvnn)) + cAgencia + cConta + cDacCC + '000'
dvcb := modulo11(s)
...
//
s    := '000' + cConta //SubStr(bldocnufinal, 3, 6) + AllTrim(Str(dvnn)) + SubStr(cAgencia, 1, 3)
dv   := modulo10(s)
...
s    := cNroDoc//SubStr(cAgencia, 4, 1) + cConta + cDacCC + '000'
dv   := modulo10(s)
RN   := RN + SubStr(s, 1, 5) + '.' + SubStr(s, 6, 5) + AllTrim(Str(dv)) + '  '
//
// 	CAMPO 4:

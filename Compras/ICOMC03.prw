...
/*
+-----------+---------+-------+-------------------------------------+------+----------------+
| Programa  | ICOMC03 | Autor | Rubens Cruz	- Anadi Soluções 		| Data | Outubro/2014	|
+-----------+---------+-------+-------------------------------------+------+----------------+
| Descricao | Consulta de UF desconsiderando paises com Suframa							    |
+-----------+-------------------------------------------------------------------------------+
| Uso       | ISAPA													 					    |
+-----------+-------------------------------------------------------------------------------+
*/   

Static _cICOMC03I := Space(40)

User Function ICOMC03()
Local cTitulo 	:= MvParDef := _cSQL := _cTab := _cRet := "", _aCad := {}
//Local cSuframa 	:= FormatIn(SuperGetMV("MV__ESTSUF",,"AC;AM"),";")
Local cEstados	:= FormatIn(SuperGetMV("MV__ESTLIS",,"SP;ES;SC;RS"),";")
...
//	_cSQL += " 		AND X5_CHAVE != 'EX' "
	_cSQL += " 		AND X5_CHAVE IN " + cEstados + "  "
	...
	AADD(_aSepara,Padr("AC",TamSX3("X5_CHAVE")[1]) + " - SUFRAMA")
	AADD(_aSepara,Padr("BR",TamSX3("X5_CHAVE")[1]  ) + " - BRASIL" )

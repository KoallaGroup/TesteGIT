...
#DEFINE MAXITEMC  050    //012                                       // Máxima de caracteres por linha de produtos/serviços
...
#DEFINE MAXVALORC 015    // 009                                            // Máximo de caracteres por linha de valores numéricos
...
Local lExistNfe := .F.     
Local lEnd		:= .F.
//Local _cPrinter := Alltrim(GetMv("MV__PDAN"))
Local _lBol := .f. // Jorge H. - Anadi - Julho/2013
...
If !AtIsRotina("U_IFATP05") .And. !AtIsRotina("U_IFATP05D") .And. !AtIsRotina("U_IFATP01") //Jorge Henrique - Anadi - Maio/2013
oDanfe:lServer := oSetup:GetProperty(PD_DESTINATION)==AMB_SERVER
	...
	Endif
//ElseIf AtIsRotina("U_IFATP05")     
//	CONOUT("ENTROU NA IMPRESSAO")
//	nDevice := Val(GetSrvProfString("RootPath", StrZero(IMP_PDF,1)) )
//	oDanfe:SetDevice(nDevice)
//	oDanfe:lViewPDF := .f.
//	oDanfe:lServer  := .t.                                              		
//	oDanfe:nDevice  := IMP_PDF
ElseIf AtIsRotina("U_IFATP05")
    nDevice := Val(GetSrvProfString("RootPath", StrZero(IMP_SPOOL,1)) )
    oDanfe:SetDevice(nDevice)
    oDanfe:lViewPDF := .f.
    oDanfe:lServer  := .t.
    oDanfe:nDevice  := IMP_SPOOL
    
    FwWriteProfString(GetPrinterSession(),"DEFAULT", Alltrim(GetMv("MV__PDAN")), .T.)   
	//fwWriteProfString(GetPrinterSession(),"DEFAULT", "\\1.0.0.31\TI", .T.)
EndIf
Private PixelX := odanfe:nLogPixelX()
Private PixelY := odanfe:nLogPixelY()

If AtIsRotina("U_IFATP05") .OR. AtIsRotina("U_IFATP05D") 
...
If lExistNfe
	...
	If AtIsRotina("U_IFATP01") .OR. AtIsRotina("U_IFATP05").OR. AtIsRotina("U_IFATP05D")  //Rubens Cruz - Dezembro/2014 - Chamada externa para impressao de boleto em PDF
		oDanfe:Print()
	Else
...
//Jorge H. - Anadi - Maio/2013
Local _lPerg := .f.

CONOUT("ENTROU NO DANFEProc")

If AtIsRotina("U_IFATP05") .Or. AtIsRotina("U_IFATP05D") .Or. AtIsRotina("U_IFATP01") 
	_lPerg := .T.
	Pergunte("NFSIGW",.f.)
	mv_par01 := SF2->F2_DOC
	mv_par02 := SF2->F2_DOC
	...
	mv_par05 := 1	
Else
	_lPerg := Pergunte("NFSIGW",.T.)
EndIf  
If _lPerg .And. ( !Empty( MV_PAR06 ) .and. MV_PAR06 == 2 .Or. ( Empty( MV_PAR06 ) )) 
	MV_PAR01X  := MV_PAR01  // INCLUIDO - APARECIDO - ANADI - 05/12/2014
	MV_PAR02X  := MV_PAR02  // INCLUIDO - APARECIDO - ANADI - 05/12/2014
...
///*****************INÍCIO - INCLUIDO - APARECIDO - ANADI 05/12/2014  ******************///
if MV_PAR04 = 1
	MVaArea := GetArea()
	DbSelectArea("SF3") // Livros Fiscais - Cabeçalho
	DbSetOrder(4)       // F3_FILIAL+F3_CLIEFOR+F3_LOJA+F3_NFISCAL+F3_SERIE                                                                                                                

	DbSelectArea("SFT") // Livros Fiscais - Itens
	DbSetOrder(4)       // FT_FILIAL+FT_TIPOMOV+FT_CLIEFOR+FT_LOJA+FT_SERIE+FT_NFISCAL+FT_CFOP                                                                                             

	DbSelectArea("SD1")
	DbSetOrder(1)  // D1_FILIAL + D1_DOC + D1_SERIE  

	cQuery	:= " SELECT * FROM " + RetSqlName("SF1")
	cQuery	+= " WHERE "
	cQuery	+= " F1_FILIAL = '" + xFilial("SF1") + "'  AND "
	cQuery	+= " F1_DOC >= '" + MV_PAR01X + "' AND F1_DOC <= '" + MV_PAR02X + "' AND F1_SERIE = '" + MV_PAR03 + "' "
	cQuery := ChangeQuery(cQuery) 
	dbUseArea( .T., "TOPCONN", TCGENQRY(,,cQuery),"TRB", .F., .T.)
	dbSelectArea("TRB") 
	dbGotop()             
	
	_grpNaoDelete := SuperGetMV("MV__GRPDEL", ,"9999")

	While !Eof()                        
		LContinua01 := .T.
		LContinua02 := .F.	
		if !EMPTY(TRB->F1_CHVNFE) .AND. TRB->F1_FORMUL = "S" .AND. EMPTY(TRB->F1__DTVTES) 
			if (TRB->F1_TIPO = "N" .AND. TRB->F1_EST = "EX") .OR. (TRB->F1_TIPO = "D")
				DbSelectArea("SD1")
				if DBSeek(xFilial("SD1") + TRB->F1_DOC + TRB->F1_SERIE )					
					While SD1->D1_DOC == TRB->F1_DOC .AND. SD1->D1_SERIE == TRB->F1_SERIE					
						LContinua02 := .T.                                                   
						
						if Posicione("SF4",1,xFilial("SF4") + SD1->D1_TES,"F4_ESTOQUE") = "S" 
							LContinua01 := .F.
						else
							if Posicione("SF4",1,xFilial("SF4") + SD1->D1_TES,"F4_PODER3") = "S" 
								LContinua01 := .F.
							else	
								if Posicione("SB1",1,xFilial("SB1") + SD1->D1_COD,"B1_GRUPO") $ _grpNaoDelete
									LContinua01 := .F.
									exit
								endif
							Endif								
		...
		if LContinua01 = .T. .AND. LContinua02 = .T.
			DbSelectArea("SF3") // Livros Fiscais - Cabeçalho
			If DBSeek(xFilial("SF3") + TRB->F1_FORNECE + TRB->F1_LOJA + TRB->F1_DOC + TRB->F1_SERIE)					
			    RecLock("SF3",.F.,.F.)
    		    dbDelete()
			    MSUNLOCK("SF3")
			Endif
			DbSelectArea("SFT") // Livros Fiscais - Itens
			if DBSeek(xFilial("SFT") + "E" + TRB->F1_FORNECE + TRB->F1_LOJA + TRB->F1_SERIE + TRB->F1_DOC ) 
				While FT_TIPOMOV = "E" .AND. FT_CLIEFOR = TRB->F1_FORNECE .AND. FT_LOJA = TRB->F1_LOJA .AND. FT_SERIE = TRB->F1_SERIE .AND. FT_NFISCAL = TRB->F1_DOC
				    RecLock("SFT",.F.,.F.)
	    		    dbDelete()
				    MSUNLOCK("SFT")
					DbSkip()
	...
	RestArea(MVaArea)  
Endif
///*****************FIM - INCLUIDO - APARECIDO - ANADI 05/12/2014  ******************///
...
If nFaturas > 0
	For nX := 1 To 3
		aAux := {}
		For nY := 1 To Min(12, nFaturas)   // ALTERADO - 9 para 12- APARECIDO - 01/12/2014
			Do Case
				Case nX == 1
					If nFaturas > 1
			...
			EndCase
		Next nY
		If nY <= 12          // ALTERADO - ERA 9 - APARECIDO - 01/12/2014
			For nY := 1 To 12              // ALTERADO - ERA 9 - APARECIDO - 01/12/2014
				AAdd(aAux, Space(20))
			Next nY
		EndIf
		AAdd(aFaturas, aAux)
...
/* 
------------------------------------------------------------------------------
	Trecho criado para acrescentar mensagem da tabela Z21 ao fim dos ítens
------------------------------------------------------------------------------
*/  

_cQuery := "select Z21.Z21_TEXTO1 as TXTNF1_1, "
_cQuery += "       Z21.Z21_TEXTO2 as TXTNF1_2, "
_cQuery += "       Z21.Z21_TEXTO3 as TXTNF1_3, "
_cQuery += "       Z21.Z21_TEXTO4 as TXTNF1_4, "
_cQuery += "       Z21.Z21_TEXTO5 as TXTNF1_5 "
...
_cQuery += "left join " + retSqlName("Z21") + " Z21 ON Z21.Z21_CODIGO = C5__TXTNF1 "
_cQuery += "                     AND Z21.D_E_L_E_T_ = ' ' "
...
TCQUERY _cQuery NEW ALIAS "TMPMSG" 

cAux := stripTags(trim(TMPMSG->TXTNF1_1) + " " + trim(TMPMSG->TXTNF1_2) + trim(TMPMSG->TXTNF1_3) + " " + trim(TMPMSG->TXTNF1_4) + " " + trim(TMPMSG->TXTNF1_5), .T.)
...
// oDanfe:Say(nLine+240,MAXBOXH-30,Iif( Empty(aDest[4]),"",ConvDate(aDest[4]) ),oFont08:oFont)  // COMENTADO - APARECIDO ANADI - 04/12/2014
oDanfe:Box(nLine+247,MAXBOXH-40,nLine+270,MAXBOXH+70)  // ALTERADO - APARECIDO ANADI - nLine+272 para nLine+270 
oDanfe:Say(nLine+255,MAXBOXH-30,"HORA ENTRADA/SAÍDA",oFont08N:oFont)
// oDanfe:Say(nLine+265,MAXBOXH-30,aHrEnt[01],oFont08:oFont)   // COMENTADO - APARECIDO ANADI - 04/12/2014

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Quadro fatura                                                           ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
aAux := {{  {},{},{},{},{},{},{},{},{},{},{},{}  }}  // ALTERADO - APARECIDO - ANADI - 01/12/2014
nY := 0
For nX := 1 To Len(aFaturas)
	...
	nY++
	aadd(Atail(aAux)[nY],aFaturas[nX][10]) //  INCLUIDO - APARECIDO - ANADI - 01/12/2014
	nY++
	aadd(Atail(aAux)[nY],aFaturas[nX][11]) //  INCLUIDO - APARECIDO - ANADI - 01/12/2014
	nY++
	aadd(Atail(aAux)[nY],aFaturas[nX][12])  //  INCLUIDO - APARECIDO - ANADI - 01/12/2014
	If nY >= 12     //  ALTERADO 9 para 12 - APARECIDO - ANADI - 01/12/2014
		nY := 0
	EndIf
Next nX
...
nPos1Col := 150.0  // ALTERADO DE 0 PARA 150 - APARECIDO - ANADI - 28/11/2014
nPos2Col := 0
For Nx := 1 to 11 // ALTERADO DE 0 PARA 150 - APARECIDO - ANADI - 28/11/2014
/******   COMENTADO - APARECIDO - ANADI - 28/11/2014
	oDanfe:Box(nLine+275,nBaseCol+30+nPos1Col,nLine+310,nBaseCol+115.1+nPos2Col)
	nPos1Col += 84.1
	nPos2Col += 84.1
*/
	oDanfe:Line( nLine+275 , nPos1Col , nLine+310 , nPos1Col ) // INCLUIDO - APARECIDO - ANADI - 28/11/2014
	nPos1Col += 65.5   
Next

	oDanfe:Line( nLine+275 , nPos1Col - 1.0 , nLine+310 , nPos1Col - 1.0 )  // // INCLUIDO - APARECIDO - ANADI - 28/11/2014

//Ultimo Box
// ******** COMENTADO -- APARECIDO - ANADI - 28/11/2014
// oDanfe:Box(nLine+275,nBaseCol+30+nPos1Col,nLine+310,MAXBOXH+70) // COMENTADO - ANADI - APARECIDO - 28/11/2014

nColuna := nBaseCol+36
aColTit := {}      // INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,106)  // 1 - INCLUIDO APARECIDO ANADI - 01/12/2014 
AADD(aColTit,156)  // 2 - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,220)  // 3 - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,286)  // 4 - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,356)  // 5 - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,416)  // 6 - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,486)  // 7 - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,556)  // 8 - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,616)  // 9 - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,680)  // A - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,750)  // B - INCLUIDO APARECIDO ANADI - 01/12/2014
AADD(aColTit,816)  // C - INCLUIDO APARECIDO ANADI - 01/12/2014
If Len(aFaturas) >0
	For nY := 1 To 12                  // ALTERADO 9 PARA 12 - APARECIDO - ANADI - 01/12/2014
//		oDanfe:Say(nLine+287,aColTit[nY],aAux[1][ny][1],oFont08:oFont) // ALTERADO nColuna PARA aColTit[nY] - APARECIDO - ANADI - 01/12/2014
		oDanfe:Say(nLine+287,aColTit[nY],STRZERO(nY,2,0),oFont08:oFont) // ALTERADO nColuna PARA aColTit[nY] - APARECIDO - ANADI - 01/12/2014
		oDanfe:Say(nLine+296,aColTit[nY],aAux[1][ny][2] ,oFont08:oFont) // ALTERADO nColuna PARA aColTit[nY] - APARECIDO - ANADI - 01/12/2014
		oDanfe:Say(nLine+305,aColTit[nY],aAux[1][ny][3] ,oFont08:oFont) // ALTERADO nColuna PARA aColTit[nY] - APARECIDO - ANADI - 01/12/2014
		nColuna:= nColuna + 60.0      // ALTERADO - ERA 84.1 - APARECIDO - ANADI - 01/12/2014
	Next nY
Endif
...
oDanfe:Line( 353, 160, 380, 160)       // INCLUIDO - APARECIDO ANADI - 02-12-2014
oDanfe:Line( 353, 621, 380, 621)       // INCLUIDO - APARECIDO ANADI - 02-12-2014
oDanfe:Line( 353, 652, 380, 652)       // INCLUIDO - APARECIDO ANADI - 02-12-2014
oDanfe:Line( 353, 702, 380, 702)       // INCLUIDO - APARECIDO ANADI - 02-12-2014
oDanfe:Line( 353, 790, 380, 790)       // INCLUIDO - APARECIDO ANADI - 02-12-2014
oDanfe:Line( 353, 870, 380, 870)       // INCLUIDO - APARECIDO ANADI - 02-12-2014

// oDanfe:Box(nLine+429,nBaseCol+30,nLine+454,232)              // COMENTADO - APARECIDO ANADI - 03/12/2014
// oDanfe:Say(nLine+437,nBaseTxt,"QUANTIDADE",oFont08N:oFont)              // COMENTADO - APARECIDO ANADI - 03/12/2014
// oDanfe:Say(nLine+447,nBaseTxt,aTransp[11],oFont08:oFont)              // COMENTADO - APARECIDO ANADI - 03/12/2014
// oDanfe:Box(nLine+429,232,nLine+454,352)              // COMENTADO - APARECIDO ANADI - 03/12/2014

////inicio inclusão APARECIDO ANADI   //////
_aArea := GetArea()
cImprimir := ""
nVol := 0
if oIdent:_TpNf:Text = "1"
	...
	cQuery	+= " WHERE "
	cQuery	+= " F2_CHVNFE = '" + SubStr(oNF:_InfNfe:_ID:Text,4) + "'"
	cQuery := ChangeQuery(cQuery) 
	dbUseArea( .T., "TOPCONN", TCGENQRY(,,cQuery),"TRB", .F., .T.)
	dbSelectArea("TRB") 
	dbGotop()
	if !Eof()
		cImprimir:= iif(TRB->F2_VOLUME1 == 0, "", STR(TRB->F2_VOLUME1,6,0)) + " " + TRB->F2_ESPECI1 + " " + ;
		            iif(TRB->F2_VOLUME2 == 0, "", STR(TRB->F2_VOLUME2,6,0)) + " " + TRB->F2_ESPECI2 + " " + ; 
					iif(TRB->F2_VOLUME3 == 0, "", STR(TRB->F2_VOLUME3,6,0)) + " " +  TRB->F2_ESPECI3 + " " + ;
					iif(TRB->F2_VOLUME4 == 0, "", STR(TRB->F2_VOLUME4,6,0)) + " " +  TRB->F2_ESPECI4 + " " 
		nVol := TRB->F2_VOLUME1 + TRB->F2_VOLUME2 + TRB->F2_VOLUME3 + TRB->F2_VOLUME4 
		if nVol = 0
			cImprimir := ""
		endif
	endif
...
Else
	cQuery	:= " SELECT * FROM " + RetSqlName("SF1")
	cQuery	+= " WHERE "
	cQuery	+= " F1_CHVNFE = '" + SubStr(oNF:_InfNfe:_ID:Text,4) + "'"
	cQuery := ChangeQuery(cQuery) 
	dbUseArea( .T., "TOPCONN", TCGENQRY(,,cQuery),"TRB", .F., .T.)
	dbSelectArea("TRB") 
	dbGotop()
	if !Eof()
		cImprimir:= STR(TRB->F1_VOLUME1,6,0) + " " + TRB->F1_ESPECI1 + " " + ;
		            STR(TRB->F1_VOLUME2,6,0) + " " + TRB->F1_ESPECI2 + " " + ; 
					STR(TRB->F1_VOLUME3,6,0) + " " + TRB->F1_ESPECI3 + " " + ;
					STR(TRB->F1_VOLUME4,6,0) + " " + TRB->F1_ESPECI4 + " " 
		nVol := TRB->F1_VOLUME1 + TRB->F1_VOLUME2 + TRB->F1_VOLUME3 + TRB->F1_VOLUME4 
		if nVol = 0
			cImprimir := ""
		endif
	endif
...
oDanfe:Say(nLine+437,625,"MARCA",oFont08N:oFont)
oDanfe:Say(nLine+447,625,aTransp[13],oFont08:oFont)
// oDanfe:Box(nLine+429,472,nLine+454,592)
oDanfe:Say(nLine+437,660,"NUMERAÇÃO",oFont08N:oFont)
oDanfe:Say(nLine+447,660,aTransp[14],oFont08:oFont)
// oDanfe:Box(nLine+429,592,nLine+454,712)
oDanfe:Say(nLine+437,720,"PESO BRUTO",oFont08N:oFont)
oDanfe:Say(nLine+447,720,Iif(!Empty(aTransp[15]),aTransp[15],Iif(Len(aEspVol)>0 .And. Val(aEspVol[1][3])>0,Transform(Val(aEspVol[1][3]),"@E 999999.9999"),"")),oFont08:oFont)
...
oDanfe:Say(nLine+437,800,"PESO LIQUIDO",oFont08N:oFont)
oDanfe:Say(nLine+447,800,Iif(!Empty(aTransp[16]),aTransp[16],Iif(Len(aEspVol)>0 .And. Val(aEspVol[1][2])>0,Transform(Val(aEspVol[1][2]),"@E 999999.9999"),"")),oFont08:oFont)
...
aAuxCabec := {;
	...
	"VLR TOTAL",;
	"VLR DESC",;
	"",;      		//V.UNI LIQ  - ALTERADO - APARECIDO - ANADI - 03/12/2013
	"",;    		//TOTAL LIQ  - ALTERADO - APARECIDO - ANADI - 03/12/2013
	"BC.ICMS",;
	"BC.ICMS ST",;
	"VLR ICMS",;
	"VLR ICMS ST",;
...
//		oDanfe:Say(nLinha, nAuxH2, aAux[1][12][nY], oFont07:oFont)
		...
		nAuxH2 := len(aAux[1][15][nY]) + (aColProd[14][1] + ((aColProd[14][2] - aColProd[14][1]) - RetTamTex(aAux[1][15][nY], oFont07:oFont, oDanfe)))
		oDanfe:Say(nLinha, nAuxH2, aAux[1][15][nY], oFont07:oFont)

		nAuxH2 := len(aAux[1][14][nY]) + (aColProd[15][1] + ((aColProd[15][2] - aColProd[15][1]) - RetTamTex(aAux[1][14][nY], oFont07:oFont, oDanfe)))
		...
		nAuxH2 := len(STRTRAN(aAux[1][18][nY], ".00", "")) + (aColProd[18][1] + ((aColProd[18][2] - aColProd[18][1]) - RetTamTex(STRTRAN(aAux[1][18][nY], ".00", ""), oFont07:oFont, oDanfe)))
		oDanfe:Say(nLinha, nAuxH2, STRTRAN(aAux[1][18][nY], ".00", "") , oFont07:oFont)
		
		nAuxH2 := len(STRTRAN(aAux[1][19][nY], ".00", "")) + (aColProd[19][1] + ((aColProd[19][2] - aColProd[19][1]) - RetTamTex(STRTRAN(aAux[1][19][nY], ".00", ""), oFont07:oFont, oDanfe)))
		oDanfe:Say(nLinha, nAuxH2, STRTRAN(aAux[1][19][nY], ".00", "")  , oFont07:oFont)
	EndIf
...
If len(oEmitente:_xNome:Text)>45
	If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
		...
		//oDanfe:Say(nLine+130,nBaseTxt, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont) - Anadi - Jorge
		oDanfe:Say(nLine+130,nBaseTxt, "SAC: 0800-702-0801  -  sac01@isapa.com.br",oFont08N:oFont)
	Else
		...
		//oDanfe:Say(nLine+120,nBaseTxt, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont) - Anadi - Jorge
		oDanfe:Say(nLine+120,nBaseTxt, "SAC: 0800-702-0801  -  sac01@isapa.com.br",oFont08N:oFont)
	EndIf
...
Else
	...
	Else
		...
		//oDanfe:Say(nLine+110,nBaseTxt, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)  - Anadi - Jorge
		oDanfe:Say(nLine+110,nBaseTxt, "SAC: 0800-702-0801  -  sac01@isapa.com.br",oFont08N:oFont)
	EndIf
Endif
...
//Anderson Goncalves - Anadi - 07/05/2015
// Por algum motivo na impressao de um range, ele não imprime nas notas seguintes o logotipo, somente na primeira pagina,
// como solucao em outro cliente que peguei, bastou copiar e deixar pegar da estcao de trabalho
aLocalArq := StrToArray(cLogo,"\")
cLocalLg := GetTempPath()
CpyS2T(cLogo,cLocalLg,.T.)
cLogo := cLocalLg + aLocalArq[2]
// Fim

If nfolha>=1
	If lMv_Logod
//		oDanfe:SayBitmap(005,075,cLogoD,095,096)      // ALTERADO - ANADI - APARECIDO - 28/11/2014
		oDanfe:SayBitmap(005,075,cLogoD,095,096)
	Else
//		oDanfe:SayBitmap(005,075,cLogo,095,096)       // ALTERADO - ANADI - APARECIDO - 28/11/2014
		oDanfe:SayBitmap(005,075,cLogo,095,060)		
	EndIf
Endif
...
//	AADD(aColProd, {nAux, nAux + aTamCol[11]}) // "VLR LIQ"		// COMENTADO - APARECIDO - ANADI - 03/12/2014
//	nAux += aTamCol[11]											// COMENTADO - APARECIDO - ANADI - 03/12/2014
//	AADD(aColProd, {nAux, nAux + aTamCol[12]}) // "TOT LIQ"		// COMENTADO - APARECIDO - ANADI - 03/12/2014
//	nAux += aTamCol[12]											// COMENTADO - APARECIDO - ANADI - 03/12/2014

	AADD(aColProd, {"", ""}) // "VLR LIQ"						// ALTERADO - APARECIDO - ANADI - 03/12/2014
	nAux += aTamCol[11]											// ALTERADO - APARECIDO - ANADI - 03/12/2014
	AADD(aColProd, {"", ""}) // "TOT LIQ"						// ALTERADO - APARECIDO - ANADI - 03/12/2014
	nAux += aTamCol[12]											// ALTERADO - APARECIDO - ANADI - 03/12/2014
...
//	nAux += aTamCol[10]
	...
	nAux += aTamCol[10]
//	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[11])     // COMENTADO APARECIDO - ANADI - 03/12/2014
//	oDanfe:Say(nLine+462, nAux + 2,"V.UNI LIQ", oFont08N:oFont)    // COMENTADO APARECIDO - ANADI - 03/12/2014
//	nAux += aTamCol[11]   										   // COMENTADO APARECIDO - ANADI - 03/12/2014
//	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[12])     // COMENTADO APARECIDO - ANADI - 03/12/2014
//	oDanfe:Say(nLine+462, nAux + 2,"TOTAL LIQ", oFont08N:oFont)    // COMENTADO APARECIDO - ANADI - 03/12/2014
//	nAux += aTamCol[12]   										   // COMENTADO APARECIDO - ANADI - 03/12/2014
	...
	oDanfe:Say(nLine+462, nAux + 2,"VLR ICMS", oFont08N:oFont)   	// ERA "BC.ICMS ST" - ALTERADO APARECIDO ANADI - 03/12/2014
	nAux += aTamCol[14]
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[15])
	oDanfe:Say(nLine+462, nAux + 2,"BC.ICMS ST", oFont08N:oFont)     // ERA "VLR ICMS" - ALTERADO APARECIDO ANADI - 03/12/2014
	nAux += aTamCol[15]
	...
	For Nx :=1 to Len(aColProd)
		if Nx = 11 .or. Nx = 12      // INCLUIDO - APARECIDO - ANADI - 03/12/2014
			Loop		    		// INCLUIDO - APARECIDO - ANADI - 03/12/2014
		endif						// INCLUIDO - APARECIDO - ANADI - 03/12/2014
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	EndIf
	aTamCol[01]	:= 35	// COD. PROD 		- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[02]	:= 207	// DESCR PROD 		- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[03]	:= 34	// NCM/SH 			- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[04]	:= 14	// CST 				- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[05]	:= 18	// CFOP				- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[06]	:= 12	// UN				- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[07]	:= 40	// QUANT			- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[08]	:= 59	// V.UNITARIO		- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[09]	:= 48	// VLR TOTAL		* INCLUIDO - APARECIDO ANADI - 03/12/2014 
	aTamCol[10]	:= 35	// VLR DESC			- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[11]	:= 0	// 					- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[12]	:= 0 	// 					- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[13]	:= 48	// BC.ICMS			* INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[14]	:= 48	// VLR ICMS			* INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[15]	:= 48	// BC.ICMS ST		* INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[16]	:= 48	// VLR ICMS ST 		* INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[17]	:= 38	// VLR IPI			- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[18]	:= 19	// ICMS				- INCLUIDO - APARECIDO ANADI - 03/12/2014
	aTamCol[19]	:= 19	// IPI				- INCLUIDO - APARECIDO ANADI - 03/12/2014
// A Soma dos Tamanhos deve ser igual a = 770 -Se Alterar um tamanho, deve alterar outro para compensar.
Return aTamCol

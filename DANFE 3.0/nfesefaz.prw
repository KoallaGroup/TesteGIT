	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			Else
				...
				//Jorge Alves - Anadi - N„o envia CNPJ quando UF for EX
				If SA2->A2_EST == "EX"
					aadd(aDest,"" )
				Else
					aadd(aDest,AllTrim(SA2->A2_CGC))
			...
			//Grava o peso para notas sem informaÁ„o de volumes - Jorge H. - Anadi - Maio/2015
			If Empty(SF1->F1_ESPECI1)
				cEspecie := "VOLUME(S)"
				If !Empty(cEspecie)
					nScan    := aScan(aEspVol,{|x| x[1] == cEspecie})		
					If nScan == 0
					...
					Else
						aEspVol[nScan][2] += FieldGet(FieldPos("F1_VOLUME1"))
					EndIf
				EndIf
			EndIf
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN,D1_DESCICM,D1_II,D1_FORMUL,D1_VALPS3,D1_ORIGLAN,D1_VALCF3,D1_TESACLA, D1__DESPRO  %Exp:cField%
						FROM %Table:SD1% SD1
						WHERE
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				aadd(aProd,	{Len(aProd)+1,;  
					(cAliasSD1)->D1_COD ,;
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					iif(empty((cAliasSD1)->D1__DESPRO), SB1->B1_DESC, (cAliasSD1)->D1__DESPRO),;
					SB1->B1_POSIPI,;
					SB1->B1_EX_NCM,;
					(cAliasSD1)->D1_CF,;
					SB1->B1_UM,;
				...
				While !CD2->(Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
					...
					Do Case
						//Isapa - Anadi
						Case SF1->F1_EST == "EX" .And. AllTrim(CD2->CD2_IMP) == "ICM"							
							aTail(aICMS) := {	SUBSTR((cAliasSD1)->D1_CLASFIS,1,1),;
											  	SUBSTR((cAliasSD1)->D1_CLASFIS,2,2),;
											  	"3",; 
							                  	0,;// Tratamento para obter o percentual da reduÁ„o de base do icms nota interna e importacao(integracao com EIC)
												0,;
												0,;
												0,;
												0,;
...
Local cOper		:= ""
Local cCFOP		:= ""
Local cMVDevCfop	:=  AllTrim(GetNewPar("MV_DEVCFOP","")) 	// Alterado Rubens em  01/06/2015
...
If AllTrim(aNota[5]) == "D" .and. !Empty(SF1->F1__CHVMSV)
	cString += '<NFRef>'
	cString += '<refNFe>'+SF1->F1__CHVMSV+'</refNFe>'
	cString += '</NFRef>'
EndIf
...
Do Case
	...
	//ADICIONADO POR RAFAEL DOMINGUES EM 29/05/15
	Case (cVeramb >= "3.10" .and. !Empty(SF1->F1__CHVMSV)) .and. (aNota[5] $ "D" .And. SF4->F4_AJUSTE <> "S")
...
//cString += '<vUnTrib>'+ConvType((aProd[10]/aProd[12]),21,8)+'</vUnTrib>' //- Jorge Alves - Anadi
...
If  !lIssQn    
	...
	Else		
		...
		If Len(aIcms)>0 .or. (aDest[09] == "EX"	.And. Len(aIcms)>0) //Isapa
			cString += '<cpl>' 
			If  Len(aIcms)>0 
				cString += '<orig>'+ConvType(aICMS[01])+'</orig>'
			Else
			...
			ElseIf ( aCST[1] == '90' .And. (SubStr(SM0->M0_CODMUN,1,2) == "31" .And. SD2->D2_TIPO == "I" .And. SF4->F4_AJUSTE == "S"))
				cString += '<valor>0</valor>' 				
			Else
				If cVerAmb >= "3.10" .and. aCST[1] $ '51' /*.and. !Empty(aICMS[12])*/ .and. !lArt186	//Isapa
					If Len(aIcms)>0 .or. aDest[09] == "EX"	//Isapa
						cString += '<vICMSOp>0</vICMSOp>'
						cString += '<pDif>100</pDif>'
